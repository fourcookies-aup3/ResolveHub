...
const TutorialCard = ({ t }: { t: Tutorial }) => {
  const isPro = t.level === "Profi";
  return (
    <Link to={`/tutorials/${t.slug}`} className="group rounded-xl overflow-hidden border border-border bg-card shadow-card hover:shadow-glow-primary transition-smooth hover:-translate-y-1 relative">
      <div className="relative aspect-video overflow-hidden">
        ...
        {isPro ? (
          <Badge className="absolute top-3 right-3 bg-gradient-cinematic text-primary-foreground border-0 gap-1">
